...
{
    Particle p;
    p.position  = position + sf::Vector2f(randomFloat(-16, 16), 0);
    p.color     = Table[mType].color;
    p.lifetime  = Table[mType].lifetime;
    if (mType == Particle::Water) {
        p.direction = randomFloat(60.f, 120.f);
    } else {
        p.direction = 90.f;
    }
...
void ParticleNode::addParticle(sf::Vector2f position, float angle)
{
    Particle p;
    p.position  = position + sf::Vector2f(randomFloat(-16, 16), 0);
    p.color     = Table[mType].color;
